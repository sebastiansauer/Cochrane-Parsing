...
```{r test-parse-review}
test_review <- parse_review(bad_url, overwrite = TRUE,
                            reviewer = "np",
...
```{r}
read_page_content_to_disk(review_url = mindfulness_url)
```


Parse all to disk, according to name of reviewer:


## Test it
```{r}
dois_by_reviewer_sample_test <- 
dois_by_reviewer_sample %>% 
  slice(1)

page_page_content_to_disk(review_url = dois_by_reviewer_sample_test$url[1],
                          output_dir = glue(
                            "output/}dois_by_reviewer_sample_test$reviewer[1]}")
...
<!-- ```{r} -->
<!-- map2(.x = dois_by_reviewer_sample_test$url, -->
<!--      .y = dois_by_reviewer_sample_test$reviewer,  -->
<!--      .f = ~ read_page_content_to_disk(review_url = .x, -->
...
<!-- ```{r} -->
<!-- map2(.x = dois_by_reviewer_sample$url, -->
